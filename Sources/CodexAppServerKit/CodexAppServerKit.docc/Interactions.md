...
A response is reserved before sending, including when several callers share a handle. A failed send is ambiguous and cannot be retried through that handle, but the interaction remains tracked until the server confirms resolution. If an unresolved request is replayed after reconnecting, its new generation-scoped handle can answer it. Both numeric and string request IDs are preserved. Replayed requests can be answered after initialization while task history is being restored; ordinary new requests still wait for recovery to finish. A `recoveryRequired` connection permits explicit user-directed recovery operations such as reading or interrupting the affected turn.
